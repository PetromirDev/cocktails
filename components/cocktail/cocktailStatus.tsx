...
const CocktailStatus: FC<{ dateModified: string; id: string }> = ({ dateModified, id }) => {
  const favourites = getFavourites();
  const currentCocktail = favourites.find((favourite) => favourite.id === id);

  return typeof currentCocktail === "object" && Date.parse(dateModified) > currentCocktail?.dateAdded ? (
